...
export function listCombinedDirectTargetRoots(
  ...
  const selectedParams: Array<number | string> = [];
  const excludedParams: Array<number | string> = [];
  ...
  if (selectedTagPredicate) {
    selectedParams.push(scanId, selectedTagDigestFlag, ...selectedTagPredicate.params);
  }
  ...
  if (excludedTagPredicate) {
    excludedParams.push(scanId, ...excludedTagPredicate.params);
  }
  ...
  return sql
    ...
    >(query, [...baseParams, scanId, scanId, ...selectedParams, ...excludedParams, ...paramsTail])
    .map(mapPlanRootRow);
}
